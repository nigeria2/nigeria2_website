...
export const Route = createFileRoute('/2027')({ component: Prediction2027 })
...
const colorOf = (party: string) => COLORS[party] ?? '#8aa093'
const NO_DATA_FILL = '#9aa79f'
...
type Meta = { weeks: string[]; election_types: string[] }
...
function Prediction2027() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/predictions/meta`)
      ...
      .then((m: Meta) => {
        ...
        if (m.election_types?.length && !m.election_types.includes('governor')) setEtype(m.election_types[0])
      })
      .catch(() => setMeta({ weeks: [], election_types: [] }))
  }, [])

  useEffect(() => {
    if (!week || !etype) return
    setRows(null)
  ...
  const typeTabs = meta?.election_types?.length ? meta.election_types : ['presidential', 'governor', 'senate']
  const raceWord = TYPE_LABEL[etype]?.toLowerCase() ?? etype

  const tabStyle = (active: boolean): React.CSSProperties => ({
    fontFamily: "'Archivo', sans-serif", fontWeight: 800, fontSize: '14px', letterSpacing: '0.02em', padding: '11px 22px',
    borderRadius: '30px', cursor: 'pointer', border: active ? '2px solid #ffe14d' : '2px solid rgba(255,255,255,0.4)',
    background: active ? '#ffe14d' : 'transparent', color: active ? '#0f4a2c' : '#eafaf0',
  })
    ...
    <div style={{ minHeight: '100vh', background: '#0d8244', fontFamily: "'Archivo', sans-serif" }}>
      ...
      <div style={{ maxWidth: '1080px', margin: '0 auto', padding: '34px 40px 0' }}>
        ...
        <p style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 600, fontSize: '16px', lineHeight: 1.5, color: '#c7e7d4', margin: '0 0 22px', maxWidth: '60ch' }}>
          Projected leading party in every state. Switch between the presidential, governorship and senate races, and step
          through measurement weeks to see how the picture is moving.
        </p>

        {/* controls */}
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '16px' }}>
          {typeTabs.map((t) => (
            <button key={t} onClick={() => setEtype(t)} style={tabStyle(etype === t)}>
              {TYPE_LABEL[t] ?? t}
            </button>
          ))}
          <select
        ...
        <div style={{ background: '#0f2a1c', marginTop: '30px' }}>
          <div style={{ maxWidth: '1080px', margin: '0 auto', padding: '40px 40px 44px' }}>
            <div style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 800, fontSize: '12px', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#ffe14d', marginBottom: '12px' }}>
              {TYPE_LABEL[etype] ?? etype} · projected states won · {weekLabel(week)}
            </div>
