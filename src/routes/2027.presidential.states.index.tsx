...
type StateRow = { geo_id: string; state: string; parties: Record<string, number>; leading_party: string; total_votes: number; lga_count: number }
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '14px' }}>
              {states.map((s) => (
                <Link key={s.geo_id} to="/2027/presidential/states/$state" params={{ state: stateSlug(s.state) }} style={{ background: '#fff', border: '1px solid #dbe4dc', borderLeft: `5px solid ${colorOf(s.leading_party)}`, borderRadius: '12px', padding: '18px 20px', textDecoration: 'none', display: 'block' }}>
                  ...
                  <div style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: '26px', color: '#0f2a1c', marginTop: '8px' }}>{s.total_votes.toLocaleString()}<span style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 700, fontSize: '13px', color: '#8aa093' }}> projected votes</span></div>
                  <div style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 700, fontSize: '12px', color: '#0f8a4a', marginTop: '10px' }}>
                    {s.lga_count} local government{s.lga_count === 1 ? '' : 's'} · view breakdown →
                  </div>
                </Link>
              ))}
            </div>
